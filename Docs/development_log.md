...
### [2026-08-25] v0.2.05 ~ v0.2.12 — PS/2 指点杆 bring-up 闭环（通信打通 + 稳定性/性能迭代）

#### v0.2.05~06 — 固件竞态修复与总线诊断探针
1. **写响应竞态修复（v13 标记）**：`ps2_gpio.c` 的 `write_awaits_resp` 改为在阻塞写**发起前**武装——设备在 ACK 位后立即发送响应字节，原"写完再武装"存在线程调度竞窗，响应字节被误入 data_queue → 命令被误判失败 → 重试风暴。
2. **POR 后 settle 时间 250ms → 1000ms**：对齐 QMK/TMK/Linux 惯例（TrackPoint 应变片滤波器清零需时）。
3. **总线空闲电平探针**：init 时回读 SCL/SDA 电平并打印。v0.2.06 探针在驱动 init 后仅 100µs 采样，受板上 RC 滤波充电影响读出假 0；v0.2.07 移至 RST 释放后采样，v0.2.09 增加每次初始化尝试前打印（配合手动按压 FPC 可实时观察接触状态）。
4. **硬件排查结论（万用表+通断实测交叉验证）**：模组 VCC=3.3V 正常、TP_RESET 600ms 低脉冲→释放高正常、SCL/SDA 连通性确认。间歇性 0/0 与时好时坏的通信确认为 FPC 接触类问题，重插/固定后消除。

#### v0.2.07~08 — 通信打通
5. **RESET 应答等待超时 300ms → 1000ms**：BAT 自检按规格允许数百毫秒，300ms 超时触发重试，而每次重发的 RTS 时钟抑制恰好踩烂正在到达的 0xAA 帧——恶性循环导致永远收不到自检结果。修复后首次读出 `0xAA` + device id，识别出 **IBM TrackPoint (0x01) / Secondary ID 0x0E / Rom 3E**。

#### v0.2.09~10 — 方向/速度/跳跃修正与误码抑制
6. **Y 轴翻转 + 速度放大**：listener 节点 `y-invert`（模组安装方向反）+ `scale-multiplier=<2>`（100Hz 默认采样率下计数偏小）。
7. **误码抑制开启并强化**（`ERROR_MITIGATION=y`）：任一轴溢出位即丢包（原要求双轴同时溢出）；超阈值判断不再要求两轴同时非零。畸形包不再进 HID。
8. **流式阶段禁止插入 0xFE 重发命令**：设备持续流式发送时注入命令会撞坏后续帧形成损坏风暴；改为直接丢帧，靠 bit-3 对齐检查在一两帧内自动重同步。

#### v0.2.11 — 日志洪水导致的卡顿修复
9. **根因**：逐包日志（每包 2~5 行 DBG/WARN/ERR）以 100Hz 报告率刷 USB CDC，同步日志发生在优先级 2 回调队列内，挤占包处理 → 处理延迟 → 设备运动累积溢出 → 更多丢弃日志的恶性循环，表现为光标卡顿/卡死。
10. **修复**：新增 `CONFIG_ZMK_INPUT_MOUSE_PS2_VERBOSE_LOG`（默认关），全部逐包日志收进开关，正常运行路径零打印开销。

#### v0.2.12 — 中断延迟削减与蓝牙参数回调
11. **ISR 热路径优化**：去掉每 bit 一次的 `k_work_cancel_delayable`（schedule 本身会重设定时器 deadline）。每字节省 11 次调度器加锁调用，缩小 BLE 射频事件抢占造成 SCL 边沿丢失的延迟窗口。
12. **蓝牙连接参数回调**：v0.2.11 曾将连接间隔固定为 7.5ms（`MAX_INT=6`），射频事件率翻倍反而加剧 PS/2 时钟沿丢失（实测蓝牙更差）；回调为 `MIN=6/MAX=12/LATENCY=4`（主机可稳定在 15ms 间隔，唤醒延迟 ≤30ms）。鼠标 HID 报告队列保持 40。

### [2026-08-25] v0.3 — 指点杆功能发布：调试通过，性能仍需优化

1. **功能状态**：PS/2 TrackPoint 在 USB 与 BLE 双通道下均可正常移动与点击（左/中/右三键），方向正确、速度可用，误码包不再透传到 HID。
2. **已知限制（性能仍需优化）**：
   - 快速移动或系统负载高时仍偶发丢帧 → 设备侧短暂溢出 → 光标瞬时停顿（USB 较轻，BLE 明显）。机理已定位：BLE 射频事件/USB 中断抢占 CPU 造成 SCL 时钟沿丢失 → 帧损坏 → 设备运动累积器饱和带溢出位发送。
   - 后续候选优化方向：a) GPIOTE PORT 模式/定时器时间戳捕获替代纯边沿中断计数；b) USB 有线连接期间自动挂起 BLE；c) TrackPoint 硬件灵敏度/采样率进一步调优；d) cb workqueue 与 BLE LL 的优先级隔离复核。
   - 调试建议：v0.3 固件下若复现停顿，串口出现的每条 `Overflow bit set` 均对应一次真实丢帧事件，可用于量化抢占影响。
3. **固件产物**：归档 `thinkpad_wireless_BMD340_app_v0.3.uf2` 与 `thinkpad_wireless_BMD340_full_v0.3.hex`（内容等同 v0.2.12，按发布版本重新命名）。
