...
3. **配置与验证方法**：
   ...
   * **PowerShell 快速检查与清除脚本**：
     ...
     Get-ChildItem -Recurse -File -Exclude *.pdf, *.xlsx, *.png, *.jpeg, *.zip | ForEach-Object {
         $bytes = [System.IO.File]::ReadAllBytes(# ThinkPad 无线键盘开发与重构日志 (Development & Refactoring Log)
...
.FullName)
         if ($bytes.Length -ge 3 -and $bytes[0] -eq 239 -and $bytes[1] -eq 187 -and $bytes[2] -eq 191) {
             Write-Host "Found BOM in: $(# ThinkPad 无线键盘开发与重构日志 (Development & Refactoring Log)
...
.FullName)"
             ...
             [System.IO.File]::WriteAllBytes(# ThinkPad 无线键盘开发与重构日志 (Development & Refactoring Log)
