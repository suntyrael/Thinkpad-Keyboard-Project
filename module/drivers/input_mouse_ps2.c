...
static const struct zmk_mouse_ps2_config zmk_mouse_ps2_config = {
    ...
    .sampling_rate = DT_INST_PROP_OR(0, sampling_rate,
                                     MOUSE_PS2_CMD_SET_SAMPLING_RATE_DEFAULT),
    .tp_press_to_select = DT_INST_PROP_OR(0, tp_press_to_select, false),
    .tp_press_to_select_threshold =
        DT_INST_PROP_OR(0, tp_press_to_select_threshold, -1),
...
static const int allowed_sampling_rates[] = {
...
void zmk_mouse_ps2_activity_click_buttons(bool button_l, bool button_m,
                                          bool button_r);
...
void zmk_mouse_ps2_activity_callback(const struct device *ps2_device,
  ...
  if (data->packet_idx == 0) {
    ...
    // Do nothing
  } else if ((data->packet_mode == MOUSE_PS2_PACKET_MODE_PS2_DEFAULT &&
              data->packet_idx == 2) ||
             (data->packet_mode == MOUSE_PS2_PACKET_MODE_SCROLL &&
              data->packet_idx == 3)) {

    zmk_mouse_ps2_activity_process_cmd(
        data->packet_mode, data->packet_buffer[0], data->packet_buffer[1],
        data->packet_buffer[2], data->packet_buffer[3]);
  ...
  k_work_schedule(&data->packet_buffer_timeout,
...
void zmk_mouse_ps2_activity_resend_callback(const struct device *ps2_device) {
  ...
  LOG_WRN("Mouse movement cmd had transmission error on idx=%d",
          data->packet_idx);
...
void zmk_mouse_ps2_activity_process_cmd(zmk_mouse_ps2_packet_mode packet_mode,
  ...
  packet = zmk_mouse_ps2_activity_parse_packet_buffer(
      packet_mode, packet_state, packet_x, packet_y, packet_extra);
  ...
  zmk_mouse_ps2_activity_click_buttons(packet.button_l, packet.button_m,
                                       packet.button_r);
...
struct zmk_mouse_ps2_packet zmk_mouse_ps2_activity_parse_packet_buffer(
  ...
  if (packet_mode == MOUSE_PS2_PACKET_MODE_SCROLL) {
    // The scroll wheel value is a signed 4-bit number (-8 .. +7) stored in
    // the low nibble of the extra byte. Sign-extend bit 3 to the full int8_t.
    // (The old code built the value from bits 0-2 only and then masked off the
    // sign bit, so negative scrolls decoded as +8..+15; review 2026-08-13 1.7)
    packet.scroll = (int8_t)(packet_extra << 4) >> 4;
  }
...
static bool zmk_mouse_ps2_is_non_zero_1d_movement(int16_t speed) {
  return speed != 0;
}
...
void zmk_mouse_ps2_activity_click_buttons(bool button_l, bool button_m,
  ...
  if (config->disable_clicking != true) {
    ...
    if (buttons_pressed > 0 || buttons_released > 0) {
      ...
      // Decrement only inside the branch that actually fires, so that a lone
      // right/middle button change still reports sync=true and is flushed
      // immediately (review 2026-08-13 item 1.4). K_NO_WAIT keeps this
      // callback-workqueue context from blocking forever when the input queue
      // backs up (review 2026-08-13 item 2.5).
      ...
      if (button_l_pressed) {
        ...
        input_report_key(data->dev, INPUT_BTN_0, 1,
                         buttons_need_reporting == 0 ? true : false, K_NO_WAIT);
...
struct zmk_mouse_ps2_send_cmd_resp
zmk_mouse_ps2_send_cmd(char *cmd, int cmd_len, uint8_t *arg, int resp_len,
  ...
  if (resp_len > sizeof(resp.resp_buffer)) {
    resp.err = -11;
    snprintf(resp.err_msg, sizeof(resp.err_msg),
             "Response can't be longer than the resp_buffer (%zu)",
             sizeof(resp.resp_buffer));
  ...
  if (pause_reporting == true && data->activity_reporting_on == true) {
    ...
    if (resp.err) {
      snprintf(resp.err_msg, sizeof(resp.err_msg),
               "Could not disable data reporting (%d)", resp.err);
    }
  }
  ...
  if (resp.err == 0) {
    ...
    for (int i = 0; i < cmd_bytes; i++) {
      ...
      if (resp.err) {
        snprintf(resp.err_msg, sizeof(resp.err_msg),
                 "Could not send cmd byte %d/%d (%d)", i + 1, cmd_bytes,
                 resp.err);
        break;
      }
    }
  ...
  if (resp.err == 0 && arg != NULL) {
    ...
    if (resp.err) {
      snprintf(resp.err_msg, sizeof(resp.err_msg), "Could not send arg (%d)",
               resp.err);
    }
  }
...
int zmk_mouse_ps2_array_get_elem_index(int elem, const int *array,
                                       size_t array_size) {
  int elem_index = -1;
...
int zmk_mouse_ps2_array_get_next_elem(int elem, const int *array,
                                      size_t array_size) {
...
int zmk_mouse_ps2_array_get_prev_elem(int elem, const int *array,
                                      size_t array_size) {
  int elem_index = zmk_mouse_ps2_array_get_elem_index(elem, array, array_size);
  if (elem_index == -1) {
    return -1;
...
int zmk_mouse_ps2_reset(const struct device *ps2_device) {
  struct zmk_mouse_ps2_send_cmd_resp resp =
      zmk_mouse_ps2_send_cmd(MOUSE_PS2_CMD_RESET, sizeof(MOUSE_PS2_CMD_RESET),
                             NULL, MOUSE_PS2_CMD_RESET_RESP_LEN, false);
  if (resp.err) {
...
int zmk_mouse_ps2_set_sampling_rate(uint8_t sampling_rate) {
  ...
  int rate_idx =
      zmk_mouse_ps2_array_get_elem_index(sampling_rate, allowed_sampling_rates,
  ...
  struct zmk_mouse_ps2_send_cmd_resp resp = zmk_mouse_ps2_send_cmd(
      MOUSE_PS2_CMD_SET_SAMPLING_RATE, sizeof(MOUSE_PS2_CMD_SET_SAMPLING_RATE),
      &sampling_rate, MOUSE_PS2_CMD_SET_SAMPLING_RATE_RESP_LEN, true);
  if (resp.err) {
...
int zmk_mouse_ps2_get_device_id(uint8_t *device_id) {
  struct zmk_mouse_ps2_send_cmd_resp resp = zmk_mouse_ps2_send_cmd(
      MOUSE_PS2_CMD_GET_DEVICE_ID, sizeof(MOUSE_PS2_CMD_GET_DEVICE_ID), NULL, 1,
      true);
  if (resp.err) {
...
int zmk_mouse_ps2_tp_get_secondary_id(uint8_t *manufacturer_id,
                                      uint8_t *secondary_id) {
  struct zmk_mouse_ps2_send_cmd_resp resp =
      zmk_mouse_ps2_send_cmd(MOUSE_PS2_CMD_TP_GET_SECONDARY_ID,
                             sizeof(MOUSE_PS2_CMD_TP_GET_SECONDARY_ID), NULL,
...
int zmk_mouse_ps2_tp_get_device_info(bool *is_tp, uint8_t *tp_manufacturer_id,
                                     uint8_t *tp_secondary_id,
                                     uint8_t *tp_rom_id, char *device_str,
                                     int device_str_size) {

  int err =
      zmk_mouse_ps2_tp_get_secondary_id(tp_manufacturer_id, tp_secondary_id);
  if (err) {
  ...
  *is_tp = true;
  ...
  char *manufacturer_str =
      zmk_mouse_ps2_get_manufacturer_str(*tp_manufacturer_id);

  snprintf(device_str, device_str_size,
           "Trackpoint by %s (0x%02X); Secondary ID: 0x%02X; Rom Version: %02X",
           manufacturer_str, *tp_manufacturer_id, *tp_secondary_id, *tp_rom_id);
...
int zmk_mouse_ps2_tp_get_config_byte(uint8_t *config_byte) {
  struct zmk_mouse_ps2_send_cmd_resp resp =
      zmk_mouse_ps2_send_cmd(MOUSE_PS2_CMD_TP_GET_CONFIG_BYTE,
                             sizeof(MOUSE_PS2_CMD_TP_GET_CONFIG_BYTE), NULL,
...
int zmk_mouse_ps2_tp_set_config_option(int config_bit, bool enabled,
  ...
  struct zmk_mouse_ps2_send_cmd_resp resp = zmk_mouse_ps2_send_cmd(
      ...
      sizeof(MOUSE_PS2_CMD_TP_SET_CONFIG_BYTE), &config_byte,
      MOUSE_PS2_CMD_TP_SET_CONFIG_BYTE_RESP_LEN, true);
  if (resp.err) {
    LOG_ERR("Could not set trackpoint %s to %s", descr,
            enabled ? "enabled" : "disabled");
    return resp.err;
  }

  LOG_INF("Successfully set config option %s to %s", descr,
          enabled ? "enabled" : "disabled");
...
int zmk_mouse_ps2_tp_press_to_select_set(bool enabled) {
  int err = zmk_mouse_ps2_tp_set_config_option(
      MOUSE_PS2_TP_CONFIG_BIT_PRESS_TO_SELECT, enabled, "Press To Select");

  return err;
}

int zmk_mouse_ps2_tp_invert_x_set(bool enabled) {
  int err = zmk_mouse_ps2_tp_set_config_option(MOUSE_PS2_TP_CONFIG_BIT_INVERT_X,
                                               enabled, "Invert X");
...
int zmk_mouse_ps2_tp_invert_y_set(bool enabled) {
  int err = zmk_mouse_ps2_tp_set_config_option(MOUSE_PS2_TP_CONFIG_BIT_INVERT_Y,
                                               enabled, "Invert Y");
...
int zmk_mouse_ps2_tp_swap_xy_set(bool enabled) {
  int err = zmk_mouse_ps2_tp_set_config_option(MOUSE_PS2_TP_CONFIG_BIT_SWAP_XY,
                                               enabled, "Swap XY");
...
int zmk_mouse_ps2_tp_sensitivity_get(uint8_t *sensitivity) {
  struct zmk_mouse_ps2_send_cmd_resp resp =
      zmk_mouse_ps2_send_cmd(MOUSE_PS2_CMD_TP_GET_SENSITIVITY,
                             sizeof(MOUSE_PS2_CMD_TP_GET_SENSITIVITY), NULL,
...
int zmk_mouse_ps2_tp_sensitivity_set(int sensitivity) {
  ...
  struct zmk_mouse_ps2_send_cmd_resp resp =
      zmk_mouse_ps2_send_cmd(MOUSE_PS2_CMD_TP_SET_SENSITIVITY,
                             sizeof(MOUSE_PS2_CMD_TP_SET_SENSITIVITY), &arg,
...
int zmk_mouse_ps2_tp_negative_inertia_get(uint8_t *neg_inertia) {
  struct zmk_mouse_ps2_send_cmd_resp resp =
      zmk_mouse_ps2_send_cmd(MOUSE_PS2_CMD_TP_GET_NEG_INERTIA,
                             sizeof(MOUSE_PS2_CMD_TP_GET_NEG_INERTIA), NULL,
...
int zmk_mouse_ps2_tp_neg_inertia_set(int neg_inertia) {
  ...
  struct zmk_mouse_ps2_send_cmd_resp resp =
      zmk_mouse_ps2_send_cmd(MOUSE_PS2_CMD_TP_SET_NEG_INERTIA,
                             sizeof(MOUSE_PS2_CMD_TP_SET_NEG_INERTIA), &arg,
...
int zmk_mouse_ps2_tp_value6_upper_plateau_speed_set(int value6) {
  ...
  if (value6 < MOUSE_PS2_CMD_TP_SET_VALUE6_UPPER_PLATEAU_SPEED_MIN ||
    ...
    LOG_ERR("Invalid value6 upper plateau speed value %d. Min: %d; Max: %d",
            value6, MOUSE_PS2_CMD_TP_SET_VALUE6_UPPER_PLATEAU_SPEED_MIN,
...
int zmk_mouse_ps2_tp_pts_threshold_get(uint8_t *pts_threshold) {
  struct zmk_mouse_ps2_send_cmd_resp resp =
      zmk_mouse_ps2_send_cmd(MOUSE_PS2_CMD_TP_GET_PTS_THRESHOLD,
                             sizeof(MOUSE_PS2_CMD_TP_GET_PTS_THRESHOLD), NULL,
...
int zmk_mouse_ps2_tp_pts_threshold_set(int pts_threshold) {
  ...
  if (pts_threshold < MOUSE_PS2_CMD_TP_SET_PTS_THRESHOLD_MIN ||
    ...
    LOG_ERR("Invalid press-to-select threshold value %d. Min: %d; Max: %d",
            pts_threshold, MOUSE_PS2_CMD_TP_SET_PTS_THRESHOLD_MIN,
  ...
  struct zmk_mouse_ps2_send_cmd_resp resp =
      zmk_mouse_ps2_send_cmd(MOUSE_PS2_CMD_TP_SET_PTS_THRESHOLD,
                             sizeof(MOUSE_PS2_CMD_TP_SET_PTS_THRESHOLD), &arg,
...
int zmk_mouse_ps2_settings_save_setting(char *setting_name, const void *value,
                                        size_t val_len) {
  char setting_path[40];
  snprintf(setting_path, sizeof(setting_path), "%s/%s",
           MOUSE_PS2_SETTINGS_SUBTREE, setting_name);
...
static void zmk_mouse_ps2_settings_save_work(struct k_work *work) {
  ...
  zmk_mouse_ps2_settings_save_setting(MOUSE_PS2_ST_TP_SENSITIVITY,
                                      &data->tp_sensitivity,
                                      sizeof(data->tp_sensitivity));
  zmk_mouse_ps2_settings_save_setting(MOUSE_PS2_ST_TP_NEG_INERTIA,
                                      &data->tp_neg_inertia,
                                      sizeof(data->tp_neg_inertia));
  zmk_mouse_ps2_settings_save_setting(MOUSE_PS2_ST_TP_VALUE6, &data->tp_value6,
                                      sizeof(data->tp_value6));
  zmk_mouse_ps2_settings_save_setting(MOUSE_PS2_ST_TP_PTS_THRESHOLD,
...
#if IS_ENABLED(CONFIG_SETTINGS)
  int ret = k_work_reschedule(&zmk_mouse_ps2_save_work,
...
static int zmk_mouse_ps2_settings_restore(const char *name, size_t len,
  ...
  if (data->is_trackpoint == false) {
    LOG_INF("Mouse device is not a trackpoint. Not restoring setting %s.",
            name);
  ...
  if (strcmp(name, MOUSE_PS2_ST_TP_SENSITIVITY) == 0) {
    ...
    if (config->tp_sensitivity != -1) {
      LOG_WRN("Not restoring runtime settings for %s with value %d, because "
              "deviceconfig "
...
int zmk_mouse_ps2_settings_init() {
  ...
  k_work_init_delayable(&zmk_mouse_ps2_save_work,
...
static void zmk_mouse_ps2_init_thread(const struct device *dev, int unused);
...
static int zmk_mouse_ps2_init(const struct device *dev) {
  ...
  k_thread_create(
      &zmk_mouse_ps2_data.thread, zmk_mouse_ps2_data.thread_stack,
...
static void zmk_mouse_ps2_init_thread(const struct device *dev, int unused) {
  ...
  if (config->sampling_rate != MOUSE_PS2_CMD_SET_SAMPLING_RATE_DEFAULT) {
    ...
    if (err) {
      LOG_ERR("Could not set sampling rate to %d: %d", config->sampling_rate,
              err);
      return;
    }
  }
  ...
  if (data->is_trackpoint == true) {
    ...
    if (config->tp_val6_upper_speed != -1) {
      LOG_INF("Setting TP value 6 upper speed plateau to %d...",
  ...
  k_work_init_delayable(&data->packet_buffer_timeout,
                        zmk_mouse_ps2_activity_packet_timout);

  return;
}

// Power-On-Reset for trackpoints (and possibly other devices).
// From the `IBM TrackPoint System Version 4.0 Engineering Specification`:
...
//
// The module runs its OWN power-on POR (finishing with the BAT self-test
// result 0xAA + device id ~600ms after release) as long as it is NOT held in
// a hardware reset. The v5 strategy kept RST LOW forever, which pins the
// module in reset so it never completes its diagnostic sequence and never
// sends 0xAA - exactly what a stuck boot shows.
//
// Correct handling: assert RST briefly (low) to guarantee a clean state, then
// RELEASE it to the run level (high) and hold it there. The module then runs
// its own ~600ms POR and sends 0xAA + device id without any further host
// action.
int zmk_mouse_ps2_init_power_on_reset() {
  struct zmk_mouse_ps2_data *data = &zmk_mouse_ps2_data;
  const struct zmk_mouse_ps2_config *config = &zmk_mouse_ps2_config;
  ...
  LOG_INF("Performing Power-On-Reset on pin P%d.%02d...",
          config->rst_gpio_port_num, config->rst_gpio.pin);

  if (data->rst_gpio.port == NULL) {
    data->rst_gpio = config->rst_gpio;
  }

  // Assert reset: drive the pin LOW (physical). The board pulls RST high, so
  // low is the active/asserted level that holds the module in reset.
  ...
  if (err) {
    LOG_ERR("Failed Power-On-Reset: Failed to release RST GPIO pin to "
            "high (err %d)",
            err);
    return err;
  }
  ...
  uintptr_t base =
      (config->rst_gpio_port_num == 0) ? 0x50000000UL : 0x50000300UL;
  uint32_t pincnf = *(
      volatile uint32_t *)(base + 0x700 + 4UL * (uint32_t)config->rst_gpio.pin);
  LOG_INF("RST pin P%d.%02d released high: raw=%d PIN_CNF=0x%08x (expect "
          "raw=1, DIR=output)",
          config->rst_gpio_port_num, config->rst_gpio.pin, raw, pincnf);
#else
  LOG_INF("RST pin P%d.%02d released high: raw=%d (expect raw=1)",
...
DEVICE_DT_INST_DEFINE(0, &zmk_mouse_ps2_init, NULL, &zmk_mouse_ps2_data,
                      &zmk_mouse_ps2_config, POST_KERNEL,
                      ZMK_MOUSE_PS2_INIT_PRIORITY, NULL);
