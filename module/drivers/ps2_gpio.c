...
#define PS2_GPIO_TIMEOUT_WRITE_SCL_START                                       \
  K_USEC(PS2_GPIO_TIMING_SCL_INHIBITION_RESP_MAX)

#define PS2_GPIO_WRITE_INHIBIT_SLC_DURATION                                    \
  K_USEC(PS2_GPIO_TIMING_SCL_INHIBITION)
...
struct ps2_gpio_data {
  ...
  char data_queue_buffer[PS2_GPIO_DATA_QUEUE_SIZE *
...
K_THREAD_STACK_DEFINE(ps2_gpio_work_queue_cb_stack_area,
...
// host reads DATA on the FALLING edge of SCL (when SCL transitions from HIGH
// to LOW). SCL is HIGH during bus idle, so the 1st clock falling edge fires
// immediately on transmission start, capturing all 11 clocks without missing
// the start bit. Writes use the opposite convention: the host changes DATA
// only while CLK is LOW and the device reads on the RISING edge.
int ps2_gpio_set_scl_callback(bool enabled, bool rising_edge) {
...
void ps2_gpio_interrupt_log_get_pos_str(int pos, char *pos_str,
                                        int pos_str_size);
...
static int ps2_gpio_init(const struct device *dev) {
  ...
  k_work_init_delayable(&data->write_inhibition_wait,
                        ps2_gpio_write_inhibition_wait);
