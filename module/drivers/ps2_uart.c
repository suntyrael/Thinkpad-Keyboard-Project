...
#define PS2_UART_TIMEOUT_WRITE_SCL_START                                       \
  K_USEC(PS2_UART_TIMING_SCL_INHIBITION_RESP_MAX)
...
struct ps2_uart_data {
  ...
  char data_queue_buffer[PS2_UART_DATA_QUEUE_SIZE *
...
K_THREAD_STACK_DEFINE(ps2_uart_work_queue_stack_area,
                      PS2_UART_WORK_QUEUE_STACK_SIZE);
static struct k_work_q ps2_uart_work_queue;

K_THREAD_STACK_DEFINE(ps2_uart_work_queue_cb_stack_area,
...
int ps2_uart_set_scl_callback_enabled(bool enabled) {
  ...
  if (enabled) {
    err = gpio_pin_interrupt_configure_dt(&config->scl_gpio,
...
int ps2_uart_get_pinctrl_port_pin(const struct pinctrl_dev_config *pcfg,
                                  uint8_t state_id, uint8_t pin_id,
                                  int *res_port, int *res_pin) {

#if IS_ENABLED(CONFIG_SOC_SERIES_NRF52X)

  if (state_id >= pcfg->state_cnt) {
    LOG_ERR("Could not retrieve pinctrl pin config, because there is no "
            "state_id %d ",
            state_id);
    return -1;
  }
  ...
  if (pin_id >= state->pin_cnt) {
    LOG_ERR(
        "Could not retrieve pinctrl pin config, because there is no pin_id %d ",
        pin_id);
    return -1;
  }
  ...
  *res_pin = PINCTRL_PSEL_NRF_EXTRACT_PIN(pin_psel);
  ...
  LOG_ERR("Could not retrieve pinctrl pin config, because the code is only "
          "designed to run on "
...
int ps2_uart_write_byte_await_response(uint8_t byte) {
  ...
  err = k_sem_take(&data->write_awaits_resp_sem,
                   PS2_UART_TIMEOUT_WRITE_AWAIT_RESPONSE);
...
void ps2_uart_write_scl_interrupt_handler_async(const struct device *dev,
                                                struct gpio_callback *cb,
                                                uint32_t pins) {
  struct ps2_uart_data *data = &ps2_uart_data;
...
static int ps2_uart_init(const struct device *dev) {
  ...
  int pinctrl_port = -1;
  int pinctrl_pin = -1;
  ps2_uart_get_pinctrl_port_pin(config->pcfg, PINCTRL_STATE_DEFAULT, 1,
                                &pinctrl_port, &pinctrl_pin);

  LOG_INF("Initializing ps2_uart driver with pins... SCL: P%d.%02d; SDA: "
          "P%d.%02d; SDA Pinctrl: "
          "P%d.%02d",
          config->scl_gpio_port_num, config->scl_gpio.pin,
          config->sda_gpio_port_num, config->sda_gpio.pin, pinctrl_port,
          pinctrl_pin);
...
static int ps2_uart_init_gpio(void) {
  ...
  gpio_init_callback(&data->scl_cb_data,
                     ps2_uart_write_scl_interrupt_handler_blocking,
                     BIT(config->scl_gpio.pin));
#else
  gpio_init_callback(&data->scl_cb_data,
                     ps2_uart_write_scl_interrupt_handler_async,
                     BIT(config->scl_gpio.pin));
...
DEVICE_DT_INST_DEFINE(0, &ps2_uart_init, NULL, &ps2_uart_data, &ps2_uart_config,
                      POST_KERNEL, 80, &ps2_uart_driver_api);
