...
#define ONE_IF_DEV_OK(n)                                                       \
  COND_CODE_1(DT_NODE_HAS_STATUS(DT_INST_PHANDLE(n, device), okay), (1 +),     \
              (0 +))
...
static void
filter_with_input_config(const struct input_listener_ps2_config *cfg,
                         struct input_event *evt) {
  if (!evt->dev) {
    return;
  }
...
static void input_handler_ps2(const struct input_listener_ps2_config *config,
  ...
  if (evt->sync) {
    if (data->mouse.wheel_data.mode == INPUT_LISTENER_XY_DATA_MODE_REL) {
      zmk_hid_mouse_scroll_set(data->mouse.wheel_data.x,
                               data->mouse.wheel_data.y);
    }
...
void zmk_input_listener_ps2_layer_toggle_activate_layer(struct k_work *item) {
  ...
  struct input_listener_ps2_data *data = CONTAINER_OF(
      d_work, struct input_listener_ps2_data, layer_toggle_activation_delay);
  const struct input_listener_ps2_config *config = data->dev->config;

  int64_t current_time = k_uptime_get();
  int64_t last_mv_within_ms =
      current_time - data->layer_toggle_last_mouse_package_time;
  ...
  if (last_mv_within_ms <= config->layer_toggle_timeout_ms * 0.1) {
    ...
    LOG_INF("Not activating mouse layer %d, because last mouse activity was "
            "%lldms ago",
...
void zmk_input_listener_ps2_layer_toggle_deactivate_layer(struct k_work *item) {
  ...
  struct input_listener_ps2_data *data = CONTAINER_OF(
      d_work, struct input_listener_ps2_data, layer_toggle_deactivation_delay);
  const struct input_listener_ps2_config *config = data->dev->config;

  LOG_INF("Deactivating layer %d due to mouse activity...",
          config->layer_toggle);
      ...
      (                                                                        \
          static const struct input_listener_ps2_config config_##n =           \
              {                                                                \
                  ...
                  .layer_toggle_delay_ms =                                     \
                      DT_INST_PROP(n, layer_toggle_delay_ms),                  \
                  .layer_toggle_timeout_ms =                                   \
                      DT_INST_PROP(n, layer_toggle_timeout_ms),                \
              };                                                               \
          ...
          void input_handler_ps2_##n(struct input_event *evt,                  \
            ...
            input_handler_ps2(&config_##n, &data_##n, evt);                    \
                                  ...
                                  input_handler_ps2_##n, NULL);                \
                                                                               \
          static int zmk_input_listener_ps2_init_##n(                          \
              const struct device *dev) {                                      \
            ...
            return 0;                                                          \
                                                                               ...
                                                                               \
          DEVICE_DT_INST_DEFINE(n, &zmk_input_listener_ps2_init_##n, NULL,     \
