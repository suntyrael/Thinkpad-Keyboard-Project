...
static int board_gpio_init(void) {
  ...
  /* ---- TrackPoint Reset pin: initial state = RUN ----
   * bmd340-module: TP4_RESET = P1.01 (dts rst-gpios, GPIO_ACTIVE_LOW, direct
   * wiring per BMD-340 schematic: HIGH=RUN / LOW=RESET). The PS/2 driver owns
   * this pin and performs its own reset pulse at init; this early driving
   * only keeps the TrackPoint out of reset during the boot window (equivalent
   * of the CoB v20 fix - do NOT remap to P1.09, that is DRV13 on this board). */
  gpio_pin_configure(gpio1_dev, 1, GPIO_OUTPUT_HIGH); /* P1.01 TP4_RESET (BMD-340): HIGH=RUN */

  /* ---- Input: charger interrupt (active LOW = charging) ---- */
  gpio_pin_configure(gpio1_dev, CHG_INT_PIN,
